// DO NOT EDIT: @generated by ash generator-rewrite 2.0.0
#![cfg_attr(rustfmt, rustfmt_skip)]
//![Vulkan Manual Page](https://docs.vulkan.org/refpages/latest/refpages/source/VK_EXT_external_memory_acquire_unmodified.html) · Vulkan extension
#[repr(C)]
pub struct ExternalMemoryAcquireUnmodifiedEXT {
    pub s_type: crate::vk::StructureType,
    pub p_next: *const core::ffi::c_void,
    pub acquire_unmodified_memory: crate::vk::Bool32,
}
pub const EXT_EXTERNAL_MEMORY_ACQUIRE_UNMODIFIED_SPEC_VERSION: usize = 69;
pub const EXT_EXTERNAL_MEMORY_ACQUIRE_UNMODIFIED_EXTENSION_NAME: &'static core::ffi::CStr = c"VK_EXT_external_memory_acquire_unmodified";
