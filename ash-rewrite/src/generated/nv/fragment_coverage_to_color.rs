// DO NOT EDIT: @generated by ash generator-rewrite 2.0.0
#![cfg_attr(rustfmt, rustfmt_skip)]
//![Vulkan Manual Page](https://docs.vulkan.org/refpages/latest/refpages/source/VK_NV_fragment_coverage_to_color.html) · Vulkan extension
#[repr(C)]
pub struct PipelineCoverageToColorStateCreateInfoNV {
    pub s_type: crate::vk::StructureType,
    pub p_next: *const core::ffi::c_void,
    pub flags: crate::vk::PipelineCoverageToColorStateCreateFlagsNV,
    pub coverage_to_color_enable: crate::vk::Bool32,
    pub coverage_to_color_location: u32,
}
#[repr(transparent)]
pub struct PipelineCoverageToColorStateCreateFlagsNV(pub(crate) i32);
pub const NV_FRAGMENT_COVERAGE_TO_COLOR_SPEC_VERSION: usize = 69;
pub const NV_FRAGMENT_COVERAGE_TO_COLOR_EXTENSION_NAME: &'static core::ffi::CStr = c"VK_NV_fragment_coverage_to_color";
