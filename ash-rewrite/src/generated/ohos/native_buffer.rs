// DO NOT EDIT: @generated by ash generator-rewrite 2.0.0
#![cfg_attr(rustfmt, rustfmt_skip)]
//![Vulkan Manual Page](https://docs.vulkan.org/refpages/latest/refpages/source/VK_OHOS_native_buffer.html) · Extension `VK_OHOS_native_buffer`
#[repr(C)]
pub struct NativeBufferOHOS {
    pub s_type: crate::vk::StructureType,
    pub p_next: *const core::ffi::c_void,
    pub handle: *mut crate::vk::OHBufferHandle,
}
#[repr(C)]
pub struct SwapchainImageCreateInfoOHOS {
    pub s_type: crate::vk::StructureType,
    pub p_next: *const core::ffi::c_void,
    pub usage: crate::vk::SwapchainImageUsageFlagsOHOS,
}
#[repr(C)]
pub struct PhysicalDevicePresentationPropertiesOHOS {
    pub s_type: crate::vk::StructureType,
    pub p_next: *mut core::ffi::c_void,
    pub shared_image: crate::vk::Bool32,
}
#[repr(transparent)]
pub struct SwapchainImageUsageFlagsOHOS(pub(crate) i32);
#[repr(transparent)]
pub struct SwapchainImageUsageFlagBitsOHOS(pub(crate) i32);
#[repr(transparent)]
#[allow(non_camel_case_types)]
pub struct OHBufferHandle(pub(crate) i16);
pub const OHOS_NATIVE_BUFFER_SPEC_VERSION: usize = 1;
pub const OHOS_NATIVE_BUFFER_EXTENSION_NAME: &core::ffi::CStr = c"VK_OHOS_native_buffer";
