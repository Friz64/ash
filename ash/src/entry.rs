...
impl Entry {
    ...
    ///
    /// No Vulkan functions loaded directly or indirectly from this [`Entry`] may be called after it
    /// is [dropped][drop()].
    ///
    /// # Example
    ///
    ...
    /// # Safety
    ///
    /// The resulting [`Instance`] **must** be destroyed through [`Instance::destroy_instance()`]
    /// (_not_ [dropped][drop()]) before the [`Entry`] this was called on is [dropped][drop()], in
    /// the broad sense of the word. That is, valid Vulkan usage requires the [`Instance`] to be
    /// destroyed, and it otherwise becomes invalid to use after this [`Entry`] is dropped.
    #[inline]
